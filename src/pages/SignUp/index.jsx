import React, { useState } from 'react';
import styles from './signUp.module.css';
import axios from 'axios';

const index = () => {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
 

  const token =
    'eyJhbGciOiJodHRwOi8vd3d3LnczLm9yZy8yMDAxLzA0L3htbGRzaWctbW9yZSNobWFjLXNoYTI1NiIsInR5cCI6IkpXVCJ9..1kK895_8yV35auAf6m0t1SkfDlWflIXj44p2rnmzqew';

  const handleSubmit = (e) => {
    e.preventDefault();
    
    const configuration = {
      method: 'post',
      url: 'https://miracle49-001-site1.itempurl.com/api/accounts/admins/login',
      data: {
        email,
        password,
      },
      headers: {
        Authorization: `Bearer ${token}`,
      },
    };

    axios(configuration)
      .then((result) => {
        setLogin(result.data.message);
        console.log(login);
      })
      .catch((err) => {
        console.log(err);
      });
  };

  return (
    <div className={styles.container}>
      <form className={styles.SignUpCard} onClick={(e) => handleSubmit(e)}>
        <input type="email" placeholder="user name" className={styles.input} onChange={(e) => setEmail(e.target.value)}/>
        <input type="password" placeholder="password" onChange={(e) => setPassword(e.target.value)}/>

        <label htmlFor="checkbox">
          <input type="checkbox" id="checkbox" />
          Remember Password
        </label>

        <button>Sign In</button>
      </form>
    </div>
  );
};

export default index;
